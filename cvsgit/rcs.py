...
class RCSFile(rcsparse.Sink):
    ...
    def warn(self, msg):
        """Issue a warning nessage"""
        # TODO: handle warnings differently
        sys.stderr.write('warning: in RCS file %s: %s\n' % (self.filename, msg))

    def changes(self, warn=warn):
        ...
        for revision in self.revision_trail(self.default_branch()):
            if not self.revs.has_key(revision):
                if warn:
                    warn(self, "incomplete revision trail at %s" % revision)
                break
